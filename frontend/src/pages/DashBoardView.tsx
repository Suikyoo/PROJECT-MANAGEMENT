...
export default function DashBoardView() {
  ...
  const fetchLogFn = async (projId: number) => {
    try {
      ...
      console.log("is insider: ", res);
      return 'error' in res ? { id: 0, projectId: projId, content: '' } : res;
    } catch {
      return { projectId: projId, content: '' };
    }
  };
  ...
  const fetchFeedbacksFn = async (pid: number) => {
    try {
      if (isClientMode()) return await tokenGetFeedbacksByProject(tokenId(), pid);
      return await getProjectFeedbacks(pid);
    } catch {
      return [];
    }
  };
    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div class="flex flex-col gap-3">
        <For each={phases()}>{(phase) => (
          <A href={isClientMode() ? `/client/${tokenId()}/project/${projectId()}/phase/${phase.id}` : `/insider/project/${projectId()}/phase/${phase.id}`} class="bg-[#121214] border border-[#1F1F23] p-4 rounded-lg flex flex-col sm:flex-row gap-4 justify-between sm:items-center no-underline hover:border-zinc-600 transition-colors">
            <div>
            ...
            <Show when={isSupervisor() && !isClientMode}>
              <button onClick={(e) => { e.preventDefault(); handleTogglePhase(phase.id); }} class="bg-[#27272A] border border-[#3F3F46] hover:bg-[#3F3F46] text-white text-xs py-1.5 px-3 rounded cursor-pointer transition-colors">Toggle State</button>
            </Show>
          </A>
        )}</For>
      </div>
      ...
      <h3 class="text-sm font-semibold uppercase text-zinc-400 tracking-wider mt-8 mb-4">Feedback</h3>
